      ...
      <div class="row" style="margin:20px">
        <div id="exTab2" class="container">
          ...
          @if($promo->status_promo=="draf")
            ...
            <button class="btn btn-primary" data-toggle="modal" data-target="#NW{{ $promo->id_pkp_promo  }}{{$promo->turunan}}"><i class="fa fa-paper-plane"></i> Sent</a></button>
						<!-- modal -->
            ...
            <!-- Modal Selesai -->
            <button class="btn btn-info" data-toggle="modal" data-target="#approve{{$promo->id_pkp_promoo}}{{$promo->turunan}}"><i class="fa fa-paper-plane"></i> Ajukan PROMO</a></button>
            <!-- modal -->
                  ...
                  <div class="modal-body">
                    ...
                    <div class="form-group row">
                      ...
                      <div class="col-md-9 col-sm-10 col-xs-12">
                        ...
                        <input type="hidden" value="Pengajuan Promo" name="judul" id="judul">
          ...
          @elseif($promo->status_promo=="revisi")
            @if(auth()->user()->role->namaRule === 'pv_lokal')
            <button class="btn btn-primary" data-toggle="modal" data-target="#revisi{{ $promo->id_pkp_promo  }}{{$promo->turunan}}"><i class="fa fa-paper-plane"></i> Sent</a></button>
						<!-- modal -->
          ...
          @elseif($promo->datapromoo->status_project=='revisi')
            ...
            @endif
          @endif
          <a class="btn btn-warning" href="{{route('downloadpromo',['id_pkp_promo' => $promo->id_pkp_promo, 'revisi' => $promo->revisi, 'turunan' => $promo->turunan])}}" onclick="return confirm('Print Data Promo ini ?')"><i class="fa fa-download"></i> Download/print PROMO</a>
          <a class="btn btn-danger" href="{{ route('rekappromo',$promo->id_pkp_promo)}}"><i class="fa fa-share"></i>Kembali</a>
          <br><br>
				  </div>
         </ul>
              ...
              <div class="panel-default">
                  ...
                  @if($promo->status_project=='draf')
                    ...
                    <tr>
                      ...
                      <td colspan="2"><?php $promo_idea = []; foreach ($promo1 as $key => $data)If (!$promo_idea || !in_array($data->promo_idea, $promo_idea)) {$promo_idea += array($key => $data->promo_idea);
                      if($data->turunan!=$promo->turunan){ echo": <s><font color='#6594c5'>$data->promo_idea<br></font></s>"; } if($data->turunan==$promo->turunan){ echo": $data->promo_idea<br>"; } } ?></td>
                    </tr>
                    <tr>
                      <td width="25%"><b>Dimension</td>
                      <td colspan="2"><?php $dimension = []; foreach ($promo1 as $key => $data) If (!$dimension || !in_array($data->dimension, $dimension)) { $dimension += array( $key => $data->dimension );
                      if($data->turunan!=$promo->turunan){ echo": <s><font color='#6594c5'>$data->dimension<br></font></s>"; } if($data->turunan==$promo->turunan){ echo": $data->dimension<br>"; } } ?></td>
                    </tr>
                    <tr>
