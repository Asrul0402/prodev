                            ...
                            <tr>
                              ...
                              <td colspan="2">
                                  ...
                                  <?php $dariusia = []; foreach ($pdf1 as $key => $data) If (!$dariusia || !in_array($data->dariusia, $dariusia)) { $dariusia += array( $key => $data->dariusia );} ?>
														      <tr><th style="border:none;">Age </th><th style="border:none;">@foreach($dariusia as $dariusia): {{$dariusia}}   Tahun - {{$pdf->sampaiusia}} Tahun ({{$pdf->revisi}}.{{$pdf->turunan}})<br>@endforeach</th></tr>
														      <?php $ses = []; foreach ($datases as $key => $data) If (!$ses || !in_array($data->ses, $ses)) { $ses += array( $key => $data->ses );} ?>
                                  <tr><th style="border:none;">SES </th><th style="border:none;"> @foreach($ses as $ses): {{$ses}} ({{$pdf->revisi}}.{{$pdf->turunan}})<br>@endforeach</th></tr>
														      <?php $gender = []; foreach ($pdf1 as $key => $data) If (!$gender || !in_array($data->gender, $gender)) { $gender += array( $key => $data->gender );} ?>
                                  <tr><th style="border:none;">Gender </th><th style="border:none;"> @foreach($gender as $gender): {{$gender}} ({{$pdf->revisi}}.{{$pdf->turunan}})<br>@endforeach</th></tr>
												    	  </table>
												      </td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                              ...
                              <td colspan="2">
                                <table class="table">
                                  ...
                                  @foreach($picture as $pic)<embed src="{{ Storage::url($pic->lokasi)}}" width="135px" height="140" type="">
                                  @endforeach</th></tr>
													      </table>
												      </td>
                            </tr>
